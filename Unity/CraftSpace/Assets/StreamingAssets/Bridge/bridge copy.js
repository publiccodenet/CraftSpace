...
class Bridge {
    ...
    constructor()
    {
        ...
        this.serverPollTime = 0.1;
        this.serverPollLastTime = -1;
        this.serverPolling = false;
        this.serverPollRequest = null;
        this.serverUrl = "http://localhost:7777/pump_events";
        this.serverRequest = null;
        this.serverRequestFetching = false;
    }
    ...
    serverPollingStart()
    {
        if (this.serverPolling) {
            return;
        }

        var body = "[" + this.jsToUnityEventQueue.join(',') + "]";
        ...
        fetch(this.serverRequest)
            .then(response => response.json())
            .then(event_list => {
                this.serverRequest = null;
                if (event_list.length) {
                    console.log("server response event_list:", event_list);
                    event_list.map(ev => this.distributeEvent(ev));
                }
                this.serverPolling = false;
                this.serverPollingStart();
            });
    ...
    queryObject(obj, query, callback)
    {
        ...
        var callbackID =
            callback &&
            this.makeCallbackID(callback, true);
    ...
    makeCallbackID(callback, oneTime)
    {
    ...
    distributeEvent(ev)
    {
        ...
        console.log("<========< Bridge: distributeEvent", ev);
